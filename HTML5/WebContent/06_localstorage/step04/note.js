...
function delSticky (event) {
	...
	if(event.target.nodeName=="SPAN"){
		ket = event.target.parentNode.id;
	}
	console.log("삭제할 키: "+key);
	
	//1번작업: 키에 해당하는 localStorage에서 아이템 삭제
	localStorage.removeItem(key);
	
	//2번작업: 키를 모아놓은 배열에서 삭제
	var stArr = getStArr();
	for(var i=0; i<stArr.length; i++) {
		if(key==stArr[i]){
			stArr.splice(i,1);	//배열에서 원하는 인덱스를 삭제(i부터 한개를 삭제한다)
			break;
		}
	}
	localStorage.setItem("stickyArr",JSON.stringify(stArr));

	//3번작업: 화면에서 key값을 id 속성에 할당한 li 태그를 삭제
	var li = document.getElementById(key);
	li.parentNode.removeChild(li); //li를 바로 삭제하는 것이 아니고 부모 노드로 간 다음 자식을 remove한다.
